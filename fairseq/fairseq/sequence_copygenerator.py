...
"""
Custom SequenceGenerator that also returns copy scores.
...
class SequenceCopyGenerator(object):
    def __init__(
        self,
        tgt_dict,
        beam_size=1,
        ...
        **kwargs
        ...
        def finalize_hypos(step, bbsz_idx, eos_scores, unfinalized_scores=None):
            ...
            if attn_clone is not None and copy_scores_clone is not None:
                assert copy_scores_clone.size() == attn_clone.size(), \
                    f"attn & copy scores shape mismatch " \
                    f"({copy_scores_clone.size()}, {attn_clone.size()})"
...
class CopyEnsembleModel(torch.nn.Module):
